...
(function () {
  (function () {
    ...
    var utils = {
      ...
      extend(t, s) {
        const a = arguments;
        const notCover = this.isBoolean(a[a.length - 1]) ? a[a.length - 1] : false;
        const len = this.isBoolean(a[a.length - 1]) ? a.length - 1 : a.length;
        for (let i = 1; i < len; i++) {
          const x = a[i];
          for (const k in x) {
  ...
  UE.parse.register('insertcode', function (utils) {
    const pres = this.root.getElementsByTagName('pre');
    if (pres.length) {
  ...
  UE.parse.register('table', function (utils) {
    ...
    if (tables.length) {
      ...
      utils.each(tables, (table) => {
        if (/\bsortEnabled\b/.test(table.className)) {
          utils.on(table, 'click', (e) => {
            ...
            const cell = findParentByTagName(target, ['td', 'th']);
            const table = findParentByTagName(target, 'table');
            const colIndex = utils.indexOf(table.rows[0].cells, cell);
            const sortType = table.getAttribute('data-sort-type');
            if (colIndex != -1) {
      ...
      function sortTable(table, sortByCellIndex, compareFn) {
        ...
        let trArray = [];
        const flag = rows[0].cells[0].tagName === 'TH';
        ...
        const Fn = {
          ...
          orderbyasc(td1, td2) {
            const value1 = td1.innerText || td1.textContent;
            const value2 = td2.innerText || td2.textContent;
            return value1.localeCompare(value2);
          },
          reversebyasc(td1, td2) {
            const value1 = td1.innerHTML;
            const value2 = td2.innerHTML;
            return value2.localeCompare(value1);
          },
          orderbynum(td1, td2) {
            let value1 = td1[utils.isIE ? 'innerText' : 'textContent'].match(/\d+/);
            let value2 = td2[utils.isIE ? 'innerText' : 'textContent'].match(/\d+/);
  ...
  UE.parse.register('charts', function (utils) {
    ...
    const resourceRoot = this.rootPath;
    const containers = this.root;
    let sources = null;
  ...
  UE.parse.register('background', function (utils) {
    const me = this;
    const root = me.root;
    const p = root.getElementsByTagName('p');
    let styles;
  ...
  UE.parse.register('list', function (utils) {
    ...
    const root = this.root;
    const ols = root.getElementsByTagName('ol');
    ...
    function applyStyle(nodes) {
      ...
      utils.each(nodes, (list) => {
        if (list.className && /custom_/i.test(list.className)) {
          const listStyle = list.className.match(/custom_(\w+)/)[1];
          if (listStyle == 'dash' || listStyle == 'dot') {
            utils.pushItem(
              customCss,
              `${selector} li.list-${customStyle[listStyle]}{background-image:url(${T.liiconpath}${
            ...
            utils.each(list.childNodes, (li) => {
              if (li.tagName == 'LI') {
                utils.pushItem(
                  customCss,
                  `${selector} li.list-${customStyle[listStyle]}${index}{background-image:url(${
                    T.liiconpath
                  }list-${customStyle[listStyle]}${index}.gif)}`,
                );
                index++;
              }
            });
  ...
  UE.parse.register('vedio', function (utils) {
    const video = this.root.getElementsByTagName('video');
    const audio = this.root.getElementsByTagName('audio');

    document.createElement('video');
    document.createElement('audio');
    if (video.length || audio.length) {
      const sourcePath = utils.removeLastbs(this.rootPath);
      const jsurl = `${sourcePath}/third-party/video-js/video.js`;
      const cssurl = `${sourcePath}/third-party/video-js/video-js.min.css`;
      const swfUrl = `${sourcePath}/third-party/video-js/video-js.swf`;
